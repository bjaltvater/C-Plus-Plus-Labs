...
{
	cout << setw(45) << "YEARLY QUARTERLY SALES" << endl << endl << endl;
	
	cout << setw(10) << "YEAR" << setw(14) << "Quarter 1"
...
void   getSales(SalesType  table, int&  numOfYears)
{
	...
	cin >> numOfYears;
		...
		for (int row = 0; row < numOfYears; row++)
		{
			...
			{
				cout << "Please enter the sales data for year " << table[row][0] << " quarter " << col << endl;
				cin >> table [row][col];
			}
		}
}
